...
class PriceSettingsRepository:
    ...
    async def create(self, water_price, damaged_bottle_fine) -> PriceSettings:
        # POST /admin/prices принимает подмножество полей. Записывать None в
        # NOT NULL колонки нельзя (IntegrityError), обнулять — тоже: непереданное
        # поле обязано сохранить действующее значение. Поэтому берём текущий
        # прайс и накладываем сверху только то, что пришло.
        stmt = (
            select(PriceSettings)
            .order_by(PriceSettings.created_at.desc())
            .limit(1)
